...
function googleNativeSend(action,payload=''){
  try{
    if(window.AndroidBudget&&typeof AndroidBudget.googleAction==='function'){
  ...
  }catch(e){toast('تعذر الاتصال بخدمة Google: '+e.message);return false}
...
function googleAccountAction(){googleNativeSend(wafferliGoogleState.signedIn?'signout':'signin','')}
...
window.wafferliGoogleDone=function(action,ok,message){
  const up=document.getElementById('googleBackupBtn'),down=document.getElementById('googleRestoreBtn');
...
print('Google Drive UI patch applied')
